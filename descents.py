...
class BaseDescent:
    ...
    def calc_loss(self, x: np.ndarray, y: np.ndarray) -> float:
        ...
        loss_not_normed = float(resid @ resid)
        return (1. / x.shape[0]) * loss_not_normed
...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        batch_indices = random.sample(range(x.shape[0]), self.batch_size)
        batch_x = x[batch_indices]
        batch_y = y[batch_indices]
        resid = self.predict(batch_x) - batch_y
        return (2. / self.batch_size) * (batch_x.T @ resid)
...
class Adam(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        step = -1. * self.lr() / (np.sqrt(v_hat) + self.eps) * m_hat
        self.w += step
        return step
...
class AdaMax(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        step = -1. * self.lr() * self.m / self.u / (1. - self.beta_1 ** self.iteration)
        self.w += step
        return step
